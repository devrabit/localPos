...
function moveModule(moduleKey, direction) {
  ...
  if (nextIndex < 0 || nextIndex >= moduleOrder.value.length) return
  const nextOrder = [...moduleOrder.value]
  nextOrder.splice(currentIndex, 1)
  nextOrder.splice(nextIndex, 0, moduleKey)
  moduleOrder.value = nextOrder
  guardarOrdenModulos(nextOrder)
}
    ...
    <div class="flex flex-col gap-4">
      ...
      >
        ...
        <CartPanel
          ...
          :mixed-transfer-str="carritoStore.mixedTransferStr"
          :change-minor="carritoStore.changeMinorValue"
          :cash-received-parsed="carritoStore.cashReceivedParsed"
          :mixed-transfer-parsed="carritoStore.mixedTransferParsed"
          :mixed-cash-required-minor="carritoStore.mixedCashRequiredMinor"
          :cash-ready-for-checkout="carritoStore.cashReadyForCheckout"
          :ready-for-checkout="carritoStore.readyForCheckout"
          ...
          @update:mixed-transfer-str="(v) => (carritoStore.mixedTransferStr = v)"
